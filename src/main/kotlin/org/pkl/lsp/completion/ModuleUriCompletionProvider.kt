...
class ModuleUriCompletionProvider(project: Project, private val packageUriOnly: Boolean) :
  ...
  companion object {
    ...
    private val SCHEME_ELEMENTS =
      listOf(
        ...
        CompletionItem(MODULE_PATH_SCHEME),
  ...
  override fun resolveCompletionItem(
    ...
    val getOriginalNode =
      project.virtualFileManager.get(URI(parsed.importNodeLocation.uri))?.getModule() ?: return null
    val getTargetModule =
      project.virtualFileManager.get(URI(parsed.moduleUri))?.getModule() ?: return null
    return listOf(getOriginalNode, getTargetModule).zip().thenApply { (originalModule, targetModule)
      ->
